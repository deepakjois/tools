...
def _lint_special_file(filename: str, dom: se.easy_xml.EasyXmlTree, file_contents: str, ebook_info: dict, special_file: str, self) -> list:
	"""
	...
	ebook_info: A dictionary containing several pieces of information about the ebook
	special_file: A string identifying the type of special file being checked
	self

	OUTPUTS
	A list of LintMessages representing errors found in the special file
	"""
...
def _lint_typo_checks(filename: str, dom: se.easy_xml.EasyXmlTree, file_contents: str, is_titlepage: bool, special_file: str) -> list:
	"""
	Helper function used in self.lint()
...
def _lint_xhtml_checks(filename: str, dom: se.easy_xml.EasyXmlTree, file_contents: str) -> list:
	"""
	Helper function used in self.lint()
...
def _lint_typography_checks(filename: str, dom: se.easy_xml.EasyXmlTree, file_contents: str, special_file: str) -> list:
	"""
	Helper function used in self.lint()
	...
	dom: A dom tree to check
	file_contents: The contents of the file being checked
	special_file: A string containing the type of special file, if any
...
def _lint_syntax_checks(filename: str, dom: se.easy_xml.EasyXmlTree, self, file_contents: str, ebook_info: dict, language: str) -> list:
	"""
	Helper function used in self.lint()
...
def _lint_metadata_checks(filename: str, dom: se.easy_xml.EasyXmlTree) -> list:
	"""
	Helper function used in self.lint()
...
def _lint_css_checks(filename: str, dom: se.easy_xml.EasyXmlTree) -> list:
	"""
	Helper function used in self.lint()
	...
	for root, directories, filenames in os.walk(self.path):
		...
		for filename in natsorted(filenames):
			...
			if filename.suffix == ".xhtml":
				...
				if special_file != "":
					messages = messages + _lint_special_file(filename, dom, file_contents, ebook_info, special_file, self)
				...
				messages = messages + _lint_metadata_checks(filename, dom)

				messages = messages + _lint_syntax_checks(filename, dom, self, file_contents, ebook_info, language)

				messages = messages + _lint_typography_checks(filename, dom, file_contents, special_file)
