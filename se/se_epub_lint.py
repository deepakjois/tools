...
def _lint_special_file_checks(filename: Path, dom: se.easy_xml.EasyXmlTree, file_contents: str, ebook_info: dict, special_file: str, self) -> list:
	"""
	...
	for root, directories, filenames in os.walk(self.path):
		...
		for filename in natsorted(filenames):
			...
			if filename.suffix == ".svg":
				...
				messages = messages + _lint_svg_checks(filename, file_contents, svg_dom, self, root)
				if self.cover_path and filename.name == self.cover_path.name:
